...
DEBUG_ice = True
userDefinedDICT = {"ice": ["正常", "去冰", "不冰", "不要冰塊", "不加冰塊", "不加", "不要加冰塊", "少冰", "半冰", "冰塊不要太多", "微", "正常冰", "微冰", "冰", "溫", "溫的", "溫飲", "熱飲", "熱", "熱的", "常溫", "不要冰的", "常溫", "不要冰"], "both": ["都少", "甜度冰塊都少", "都一半", "甜度冰塊都一半", "都正常", "甜度冰塊都正常", "甜度冰塊正常", "各"], "sugar": ["半糖", "半", "一半", "少糖", "不要太甜", "微糖", "微", "一點", "一點點", "無糖", "無", "不要甜", "不糖", "不加糖", "不加", "不要糖", "全糖"], "whatever": ["都可以", "隨便"], "原鄉四季": ["原鄉", "四季", "原鄉四", "四季原鄉"], "極品菁茶": ["極品", "菁茶", "極品菁", "菁茶極品"], "特級綠茶": ["特級", "綠茶", "特綠", "特級綠", "綠茶特級"], "特選普洱": ["特選", "普洱", "特選普", "普洱特選", "普洱茶"], "翡翠烏龍": ["翡翠", "烏龍", "翡烏", "翡龍", "翡翠烏", "烏龍翡翠"], "錫蘭紅茶": ["錫蘭", "紅茶", "錫紅", "錫蘭紅", "紅茶錫蘭"], "烏龍綠茶": ["烏龍", "綠茶", "烏綠", "烏龍綠", "綠茶烏龍"], "嚴選高山茶": ["嚴選", "高山茶", "嚴選高山", "山茶", "山茶嚴選", "嚴選高"]}
...
def getIce(argsLIST):
    for a in argsLIST:
        ...
        elif a in userDefinedDICT["both"]:
            return argsLIST.index(a)
        else:
            pass # 很重要不然對到第一格沒有後他就不對了 :c
    return None
...
def getResult(inputSTR, utterance, args, resultDICT):
    ...
    if utterance == "[一杯][去冰][原鄉]":
        checkResult = getIce(args)
        if checkResult != None:
            resultDICT["ice"].append(args[checkResult])
    ...
    if utterance == "[兩杯][熱的][特選普洱]，[甜度冰塊正常]":
        resultDICT["ice"].append(args[getIce(arg[0:3])])
    ...
    if utterance == "[嚴選][微][微]":
        checkResult = getIce(args)
        if checkResult != None:
            if args[checkResult].startswith("微") and args[checkResult][-1] not in ("冰", "溫", "熱"):
                resultDICT["ice"].append(args[checkResult] + "冰")
            else: resultDICT["ice"].append(args[checkResult])
